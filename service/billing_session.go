...
func (s *BillingSession) Settle(actualQuota int) error {
	...
	if !s.relayInfo.IsPlayground && tokenDelta != 0 {
		tokenErr = adjustTokenQuota(s.relayInfo, tokenDelta)
		if tokenErr != nil {
...
func (s *BillingSession) Refund(c *gin.Context) {
	...
	if s.tokenConsumed > 0 && !s.relayInfo.IsPlayground {
		if err := adjustTokenQuota(s.relayInfo, -s.tokenConsumed); err != nil {
...
func (s *BillingSession) preConsume(c *gin.Context, quota int) *types.NewAPIError {
	...
	if err := s.funding.PreConsume(effectiveQuota); err != nil {
		// 预扣费失败，回滚令牌额度
		if s.tokenConsumed > 0 && !s.relayInfo.IsPlayground {
			if rollbackErr := adjustTokenQuota(s.relayInfo, -s.tokenConsumed); rollbackErr != nil {
