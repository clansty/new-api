...
type RelayInfo struct {
	...
	TokenUnlimited    bool
	TokenCustomRatio  float64
	TokenQuotaRatio   float64
	StartTime         time.Time
...
func (info *RelayInfo) InitChannelMeta(c *gin.Context) {
	...
	if channelType == constant.ChannelTypePassThrough {
...
func (info *RelayInfo) ToString() string {
	...
	fmt.Fprintf(b, "User{ Id: %d, Email: %q, Group: %q, UsingGroup: %q, Quota: %d, AllowOverdraft: %t }, ",
		info.UserId, common.MaskEmail(info.UserEmail), info.UserGroup, info.UsingGroup, info.UserQuota, info.AllowOverdraft)
...
var streamSupportedChannels = map[int]bool{
	...
	constant.ChannelTypeAdvancedPassThrough: true,
}
...
func genBaseRelayInfo(c *gin.Context, request dto.Request) *RelayInfo {
	...
	info := &RelayInfo{
		...
		AllowOverdraft: common.GetContextKeyBool(c, constant.ContextKeyUserAllowOverdraft),
