...
type Token struct {
	...
	UsedQuotaInitialized bool           `json:"-" gorm:"default:false"`
	Group                string         `json:"group" gorm:"default:''"`
	CrossGroupRetry      bool           `json:"cross_group_retry"` // 跨分组重试，仅auto分组有效
	CustomRatio          float64        `json:"custom_ratio,omitempty" gorm:"default:0"`
...
func SearchUserTokens(userId int, keyword string, token string, offset int, limit int) (tokens []*Token, total int64, err error) {
	...
	if keyword != "" {
		...
		baseQuery = baseQuery.Where("name "+commonLikeOp+" ? ESCAPE '!'", keywordPattern)
	}
	if token != "" {
		...
		baseQuery = baseQuery.Where(commonKeyCol+" "+commonLikeOp+" ? ESCAPE '!'", tokenPattern)
	}
...
func (token *Token) Update() (err error) {
	...
	err = DB.Model(token).Select("name", "status", "expired_time", "remain_quota", "unlimited_quota",
		"model_limits_enabled", "model_limits", "allow_ips", "group", "cross_group_retry", "custom_ratio").Updates(token).Error
	return err
}
