...
func SearchChannels(keyword string, group string, model string, idSort bool) ([]*Channel, error) {
	...
	if group != "" && group != "null" {
		...
		if common.UsingMySQL {
			groupCondition = `CONCAT(',', ` + commonGroupCol + `, ',') ` + commonLikeOp + ` ?`
		} else {
			// sqlite, PostgreSQL
			groupCondition = `(',' || ` + commonGroupCol + ` || ',') ` + commonLikeOp + ` ?`
		}
		whereClause = "(id = ? OR name " + commonLikeOp + " ? OR " + commonKeyCol + " = ? OR " + baseURLCol + " " + commonLikeOp + " ?) AND " + modelsCol + ` ` + commonLikeOp + ` ? AND ` + groupCondition
		args = append(args, common.String2Int(keyword), "%"+keyword+"%", keyword, "%"+keyword+"%", "%"+model+"%", "%,"+group+",%")
	} else {
		whereClause = "(id = ? OR name " + commonLikeOp + " ? OR " + commonKeyCol + " = ? OR " + baseURLCol + " " + commonLikeOp + " ?) AND " + modelsCol + " " + commonLikeOp + " ?"
...
func (channel *Channel) EffectiveUpstreamRateMultiplier() (*float64, bool) {
	if channel.UpstreamRateMultiplier != nil {
		return channel.UpstreamRateMultiplier, false
	}
	return channel.ManualUpstreamRateMultiplier, channel.ManualUpstreamRateMultiplier != nil
}

func (channel *Channel) UpdateManualUpstreamRateMultiplier(rate *float64) error {
	if err := DB.Model(channel).Update("manual_upstream_rate_multiplier", rate).Error; err != nil {
		return err
	}
	channel.ManualUpstreamRateMultiplier = rate
	return nil
}
